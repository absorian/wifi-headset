...
static void control_transport_task(void *param)
{
	...
	ret = connect(s_sock, (struct sockaddr *)addr,
	...
	esp_event_post_to(g_main_event_loop, APP_MAIN, APP_HOST_CONNECTED, NULL,
			  0, portMAX_DELAY);
...
void control_transport_start(const host_conn_info_t *info)
{
	...
	xTaskCreate(control_transport_task, "control_transport", 2048, &addr, 0,
		    &s_task);
}
